...
class AudioFeatures(object):
    ...
    def __init__(self, mono=True, hop_length=512, sample_rate=44100, normalize_gain=False):
        """[summary]
        ...
        self.model = crema.models.chord.ChordModel()
    ...
    def madmom_features(self, fps=100):
        ...
        snovfn = np.interp(np.arange(nframes) * self.hop_length / float(self.fs), np.arange(len(snovfn)) / float(fps),
                           snovfn)
    ...
    def crema(self):
        ...
        data = self.model.outputs(y=self.audio_vector, sr=self.fs)
    ...
    def mfcc_htk(self, window_length=22050, nmfcc=13, n_mels=26, fmax=8000, lifterexp=22):
        ...
        for frame in estd.FrameGenerator(self.audio_vector, frameSize=window_length, hopSize=self.hop_length,
